...
HELP_TEXT = (
    ...
    "<code>/resetseen</code> — забыть, какие объявления уже присылались "
    "(чтобы увидеть их снова)\n\n"
    f"Лимит по умолчанию: {config.DEFAULT_LIMIT}, максимум: {config.MAX_LIMIT}.\n"
    "Объявления, которые я уже присылал в этом чате, повторно не приходят — "
    "при повторном поиске я автоматически ищу дальше, пока не наберу новые.\n"
...
@bot.message_handler(commands=["resetseen"])
def handle_reset_seen(message):
    count = history.reset(message.chat.id)
    if count:
        bot.reply_to(message, f"🗑 Забыл {count} ранее отправленных объявлений — они снова могут появиться в поиске.")
    else:
        bot.reply_to(message, "История отправленных объявлений и так пуста.")
...
def handle_parse(message):
    ...
    # exclude_urls — объявления, которые этому чату уже присылались раньше
    # (см. history.py): парсер пропустит их и будет листать дальше, пока
    # не наберёт `limit` действительно НОВЫХ объявлений. ---
    already_seen = history.get_seen(message.chat.id)
    try:
        ads = search_ads(query, limit, filters=chat_filters, progress_cb=_progress, exclude_urls=already_seen)
    ...
    # --- Шаг 2: дозапрашиваем детали ПАЧКАМИ (несколько объявлений
    # параллельно, см. config.DETAIL_CONCURRENCY) и шлём в чат сразу, как
    # только объявление готово — заметно быстрее, чем строго по одному.
    # Дополнительно: перед отправкой проверяем историю ещё раз (на случай
    # гонки между /parse и /monitor в одном чате) и отмечаем объявление как
    # отправленное — повторно оно уже не придёт. Файл olx.txt гарантированно
    # уходит в finally — даже если по пути OLX заблокирует часть запросов
    # или что-то упадёт с ошибкой. ---
    sent_ads = []
    try:
        for ad, ok in enrich_many(ads, max_workers=config.DETAIL_CONCURRENCY):
        ...
        if sent_ads:
            ...
            bot.send_message(
                ...
                "не осталось, либо все найденные объявления уже были показаны "
                "раньше. Попробуйте ослабить фильтры через /filters или "
                "сбросить историю через /resetseen.",
            )


def _format_rating_line(ad: dict) -> str:
    """Раньше, если рейтинг не удалось найти на странице, в чат всегда
    уходило "0/5" — что выглядит как реальная плохая оценка продавца, хотя
    на самом деле означает "не нашли данных". Теперь чётко разделяем:
    известный рейтинг (+ количество отзывов, если оно тоже известно) и
    "отзывов нет / не указано"."""
    rating = ad.get("seller_rating")
    reviews = ad.get("seller_reviews_count")
    if rating not in (None, ""):
        if reviews not in (None, "", 0):
            return f"{rating}/5 ({reviews} отзывов)"
        if reviews == 0:
            return f"{rating}/5 (отзывов нет)"
        return f"{rating}/5"
    if reviews == 0:
        return "нет отзывов"
    if reviews not in (None, ""):
        return f"не указан ({reviews} отзывов)"
    return "не указан"
...
def _send_txt_file(chat_id: int, ads: list):
    ...
    with open(path, "w", encoding="utf-8") as f:
        for ad in ads:
            ...
            f.write(f"Рейтинг: {_format_rating_line(ad)}\n")
            f.write("===\n")
        ...
        try:
            with open(path, "rb") as fh:
                bot.send_document(
                    ...
                    visible_file_name=config.OUTPUT_FILE,
                )
            return
        except Exception as e:
            last_err = e
            m = re.search(r"retry after (\d+)", str(e), re.I)
            wait = int(m.group(1)) + 1 if m else 3 * attempt
            log.warning("send_document не удался (попытка %s/3): %s. Жду %s сек.", attempt, e, wait)
            time.sleep(wait)

    log.error("Не удалось отправить olx.txt в чат %s после 3 попыток: %s", chat_id, last_err)
    try:
        bot.send_message(
            chat_id,
            "⚠️ Не получилось отправить файл с результатами (похоже, Telegram "
            "временно ограничил отправку). Объявления выше уже пришли, "
            "попробуйте повторить /parse чуть позже.",
        )
    except Exception:
        pass
...
def _monitor_loop(chat_id: int, query: str, snapshot_filters: dict, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        try:
            # exclude_urls — общая с /parse история отправленных объявлений
            # в этом чате: одно и то же объявление не придёт повторно ни
            # через /parse, ни через /monitor, независимо от того, кто из
            # них показал его первым.
            already_seen = history.get_seen(chat_id)
            ads = parse_olx(query, config.MONITOR_CHECK_LIMIT, filters=snapshot_filters, fetch_details=True,
                             exclude_urls=already_seen)
        ...
        if new_ads:
            ...
            for a in new_ads:
                history.mark_sent(chat_id, a.get("ad_url"))
