...
def handle_parse(message):
    ...
    # Долгий парсинг (особенно дозапрос данных о продавцах) не должен
    # выглядеть как зависший бот — периодически обновляем статус-сообщение.
    last_edit_ts = [0.0]
    ...
    def _progress(stage, current, total):
        ...
        if stage == "page":
            ...
            text = f"📥 Собираю данные о продавцах: {current}/{total}"
        try:
    ...
    try:
        ads = parse_olx(query, limit, filters=chat_filters, progress_cb=_progress)
