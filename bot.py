...
def _monitor_loop(chat_id: int, query: str, snapshot_filters: dict, stop_event: threading.Event):
    log.info("Мониторинг запущен: chat=%s query=%s", chat_id, query)
    # Загружаем уже отправленные URL из постоянного хранилища
    ...
    while not stop_event.is_set():
        ...
        # Сохраняем новые URL в хранилище
        if new_ads:
            new_urls = [a["ad_url"] for a in new_ads]
            add_seen_urls(chat_id, new_urls)
...
def handle_monitor(message):
    ...
    with _MONITORS_LOCK:
        ...
        _MONITORS[chat_id] = {"stop_event": stop_event, "thread": thread, "query": query}
        thread.start()
    ...
    bot.reply_to(
        ...
        f"Проверка каждые {config.MONITOR_INTERVAL_SECONDS // 60} мин. с текущими фильтрами (/filters).\n"
        "Чтобы остановить: /stopmonitor\n"
        "Уже отправленные объявления не будут показаны повторно.",
    )
