...
# --- Параллельность дозапроса деталей объявления (ускорение парсинга) ---
# Раньше страницы объявлений дозапрашивались строго ПО ОДНОЙ (с паузой
# DETAIL_DELAY между каждой) — это самая медленная часть парсинга, т.к. на
# каждое объявление нужен отдельный HTTP-запрос за данными продавца. Теперь
# такие запросы идут пачками по DETAIL_CONCURRENCY штук ОДНОВРЕМЕННО (в
# разных потоках), что ускоряет обработку примерно в DETAIL_CONCURRENCY раз.
# Больше значение — быстрее, но выше риск словить 403/429 от OLX за слишком
# частые запросы с одного IP. 3-5 — разумный баланс для бесплатного
# хостинга; можно переопределить переменной окружения.
